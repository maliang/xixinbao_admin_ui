...
import {
  ...
  NPagination, NModal, NForm, NFormItem, NDatePicker, useDialog
} from 'naive-ui';
import { fetchShippingOrders, updateShippingLogistics } from '@/service/api';
...
const logisticsForm = ref({
  ...
  progressList: [] as { time: string; desc: string }[]
});
...
function openLogistics(row: any) {
  ...
  // 从后端返回的数据中读取物流进度
  var progressData = row.shippingProgress || row.shipping_progress || [];
...
function addProgress() {
  if (!logisticsForm.value.newProgress) return;
  const now = new Date();
  ...
  logisticsForm.value.progressList.unshift({ time: timeStr, desc: logisticsForm.value.newProgress });
  logisticsForm.value.newProgress = '';
...
async function handleSaveLogistics() {
  ...
  const { error } = await updateShippingLogistics(editingOrderId.value, {
    ...
    progress_list: logisticsForm.value.progressList
  });
  if (!error) {
...
<template>
  <div class="p-16px">
    ...
    <!-- 物流信息弹窗 -->
    <NModal v-model:show="logisticsVisible" preset="card" title="物流信息" style="width: 560px;" :bordered="false">
      <NForm label-placement="top" size="small">
        ...
        <NFormItem label="物流进度">
          <div class="w-full">
            <NInput v-model:value="logisticsForm.newProgress" placeholder="输入新的物流进度信息" class="mb-8px" />
            ...
            <div v-for="(item, index) in logisticsForm.progressList" :key="index"
              class="flex items-start gap-8px p-10px mb-6px rounded-6px" style="background: #f9f9f9;">
              <div class="flex-1">
