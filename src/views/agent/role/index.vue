...
import { fetchAgentRoles, createAgentRole, updateAgentRole, deleteAgentRole, fetchPermissions } from '@/service/api';
import { useAuthStore } from '@/store/modules/auth';

defineOptions({ name: 'AgentRolePage' });
...
const columns = ref<DataTableColumns>([
  ...
  { title: '创建时间', key: 'createdAt', width: 160 },
  {
    ...
    render: (row) => h(NSpace, { size: 8 }, () => [
      authStore.hasPermission('agent.role.edit') ? h(NButton, { text: true, type: 'primary', onClick: () => openEdit(row) }, () => '编辑') : null,
      authStore.hasPermission('agent.role.delete') ? h(NButton, { text: true, type: 'error', onClick: () => confirmDelete(row) }, () => '删除') : null
    ])
  }
]);
...
async function loadData() {
  loading.value = true;
  const { data: resData, error } = await fetchAgentRoles();
  loading.value = false;
  if (!error) {
...
async function handleSave() {
  ...
  if (formData.value.id) {
    const { error } = await updateAgentRole(formData.value.id, payload);
    if (!error) {
    ...
    const { error } = await createAgentRole(payload);
    if (!error) {
...
function confirmDelete(row: any) {
  dialog.warning({
    ...
    onPositiveClick: async () => {
      const { error } = await deleteAgentRole(row.id);
      if (!error) {
  ...
  <div class="p-16px">
    <NCard :bordered="false" class="card-wrapper">
      <div class="flex items-center justify-between mb-16px">
        ...
        <NButton v-permission="'agent.role.create'" type="primary" @click="openCreate">
