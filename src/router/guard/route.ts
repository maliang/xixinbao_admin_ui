...
export function createRouteGuard(router: Router) {
  router.beforeEach(async (to, from) => {
    ...
    // 权限码检查：读取路由 meta.permissions 字段
    const routePermissions = (to.meta.permissions as string[]) || [];
    const hasPermissionAccess = authStore.isStaticSuper
      || !routePermissions.length
      || authStore.hasAnyPermission(routePermissions);

    // 综合判断：角色检查 AND 权限码检查
    const hasAuth = hasPermissionAccess && (authStore.isStaticSuper || !routeRoles.length || hasRole);
