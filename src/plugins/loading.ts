...
export function setupLoading() {
  ...
  // 从缓存读取站点配置（由 SystemLogo 组件写入，TTL 1天）
  let logoHtml = `<div class="w-128px h-128px">${getLogoSvg()}</div>`;
  let titleText = $t('system.title');
  try {
    ...
    if (raw) {
      ...
      if (cache.expireAt && Date.now() < cache.expireAt) {
        if (cache.adminLogo) {
          logoHtml = `<img src="${cache.adminLogo}" style="width:128px;height:128px;object-fit:contain;" alt="logo" />`;
        }
        if (cache.siteName) {
          titleText = cache.siteName;
        }
      }
    }
  } catch {
...
<div class="fixed-center flex-col bg-layout" style="${cssVars}">
  ...
  <h2 class="text-28px font-500 text-primary">${titleText}</h2>
</div>`;
