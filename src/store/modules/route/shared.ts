 ...
 *
 * @param routes Auth routes
 * @param permissions User permission codes
 * @param isSuperAdmin Whether the user is super admin
 */
 ...
 * @param permissions User permission codes
 */
function filterRouteByPermissions(route: ElegantConstRoute, permissions: string[]): ElegantConstRoute[] {
  ...
  if (filterRoute.children?.length) {
    filterRoute.children = filterRoute.children.flatMap(item => filterRouteByPermissions(item, permissions));
  }
